...
int page_alloc(struct Page **new) {
    ...
    memset((void *)page2kva(pp), 0,
           PAGE_SIZE); // memset requests virtual address!!!!
 ...
 * pointer to the page table entry for virtual address 'va'.
 *
 * Pre-Condition:
 ...
 * 12 bits) of the page table entry should be set to 'perm | PTE_C_CACHEABLE |
 * PTE_V'.
 *
 * Post-Condition:
 *   Return 0 on success
