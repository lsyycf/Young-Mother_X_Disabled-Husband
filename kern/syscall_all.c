 ...
 *  You may use the unmapped and uncached segment in kernel address space
 *(KSEG1) to perform MMIO by assigning a corresponding-lengthed data to the
 *address, or you can just simply use the io function defined in 'include/io.h',
...
static inline int is_illegal_dev_range(u_long pa, u_long len) {
    ...
    for (i = 0; i < valid_addr_space_num; i++) {
        if (target_start >= valid_addr_start[i] &&
            target_end <= valid_addr_end[i]) {
            return 0;
        }
    }
    return 1;
 ...
 * Hint:
 *  You can use 'is_illegal_va_range' to validate 'va'.
 *  You can use function 'ioread32', 'ioread16' and 'ioread8' to read data from
 * device.
 */
...
void sys_set_var(char *key, char *value, int r, int x) {
    set_env_var(curenv->env_id, key, value, r, x);
}

void sys_get_var(char *key, char *res) {
    get_env_var(curenv->env_id, key, res);
}

void sys_unset_var(char *key) { unset_env_var(curenv->env_id, key); }

void sys_print_var() { print_env_var(curenv->env_id); }

u_int sys_get_parent() { return curenv->env_parent_id; }
...
void *syscall_table[MAX_SYSNO] = {
    ...
    [SYS_get_curpath] = sys_get_curpath,
    [SYS_set_curpath] = sys_set_curpath,
    [SYS_set_var] = sys_set_var,
    [SYS_unset_var] = sys_unset_var,
    [SYS_get_var] = sys_get_var,
    [SYS_print_var] = sys_print_var,
