...
static int spawn_mapper(void *data, u_long va, size_t offset, u_int perm,
                        const void *src, size_t len) {
    u_int child_id = *(u_int *)data;
    try(syscall_mem_alloc(child_id, (void *)va, perm));
    if (src != NULL) {
        int r = syscall_mem_map(child_id, (void *)va, 0, (void *)UTEMP,
                                perm | PTE_D);
        if (r) {
...
/* Note:
 *   This function involves loading executable code to memory. After the
 * completion of load procedures, D-cache and I-cache writeback/invalidation
 * MUST be performed to maintain cache coherence, which MOS has NOT implemented.
 * This may result in unexpected behaviours on real CPUs! QEMU doesn't simulate
 * caching, allowing the OS to function correctly.
 */
int spawn(char *prog, char **argv) {
    ...
    // different size than expected), set 'r' and 'goto err' to close the file
