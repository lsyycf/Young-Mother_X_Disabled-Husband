...
void syscall_get_curpath(char *path) { msyscall(SYS_get_curpath, path); }

void syscall_set_curpath(char *path) { msyscall(SYS_set_curpath, path); }

void syscall_set_var(char *key, char *value, int r, int x) {
    msyscall(SYS_set_var, key, value, r, x);
}

void syscall_get_var(char *key, char *res) { msyscall(SYS_get_var, key, res); }

void syscall_unset_var(char *key) { msyscall(SYS_unset_var, key); }

void syscall_print_var() { msyscall(SYS_print_var); }

u_int syscall_get_parent() { return msyscall(SYS_get_parent); }